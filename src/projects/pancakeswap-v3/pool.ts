...
export function handleSwap(event: Swap) {
  const investmentId = getInvestmentId(PANCAKESWAP_V3_PROTOCOL, event.address);
  ...
  investment.meta = [
    ...
    Bytes.fromBigInt(event.params.sqrtPriceX96),
  ];
  investment.save();
}
